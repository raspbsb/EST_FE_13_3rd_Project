...
> `analyze` Edge Function은 로그인 사용자 인증, 요청 형식, GitHub URL/API 오류 및 서버 측 쿨다운을 검증합니다. 현재 요청 본문에는 `project_id`가 없고, 함수 내부에 포트폴리오 작성자 조회·대조 로직도 없습니다. 또한 GitHub 계정과 저장소 owner를 대조하는 로직은 현재 활성화된 `analyze` 함수에 없습니다. GitHub 계정 연동 관련 헬퍼와 에디터의 대조 흐름은 존재하지만, 에디터에서는 주석 처리되어 있습니다.
...
테이블 스키마 migration/DDL은 저장소에 없으므로, 아래는 실제 Supabase 쿼리·관계 선택문에서 확인되는 테이블과 **코드 관찰 관계**입니다. 단, RLS 정책 정의는 `supabase/rls_policies.sql`에서 별도로 확인됩니다.
